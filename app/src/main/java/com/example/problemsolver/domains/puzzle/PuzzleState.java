...
public class PuzzleState extends State {
    ...
    public int hashCode() {
        int hash = 3;
        hash = 13 * hash + Arrays.deepHashCode(this.tiles);
        return hash;
    }
...
//        return tilesOutOfPlace(this, goal);
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        ...
        PuzzleState state = (PuzzleState)current;
        PuzzleState state2 = (PuzzleState)goal;
        for(int i=0; i < state.tiles.length;i++) {
            for(int j = 0; j < state.tiles[0].length;j++) {
                if   (state.tiles[i][j] != 0 && state.tiles[i][j] != state2.tiles[i][j] )
                {
    ...
    public static int sumManhattan(State current, State goal) {
        ...
        for(int i=1; i< 9; i++) {
            ...
            sum += Math.abs(state.getLocation(i).getColumn()- state2.getLocation(i).getColumn())
                    + Math.abs(state.getLocation(i).getRow()- state2.getLocation(i).getRow());
        }
