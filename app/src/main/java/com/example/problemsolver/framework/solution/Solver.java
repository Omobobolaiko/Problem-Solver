...
public abstract class Solver {
    ...
    private Vertex search(Vertex s) {
        ...
        statistics.putStat(QUEUE_SIZE, max);
        statistics.incrStat(QUEUE_SIZE);
        ...
        while (!queue.isEmpty()){
            ...
            // if (!problem.getFinalState() = null) {
            if (u.getData().equals(problem.getFinalState()))  {
                return u;
            }
            statistics.incrStat(NUM_OPS);
            for(Vertex successor : expand(u)){

                if (!vertices.containsKey(successor)) {
                    vertices.put(successor, u);
                    successor.setDistance(u.getDistance() + 1);
                    successor.setPredecessor(u);
                    queue.add(successor);
                    statistics.incrStat(NUM_OPS);
